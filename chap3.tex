...
Once each course is represented by a standardized text document, the next step is to convert this textual information into a semantically rich, numerical format that machine learning models can process. This is achieved through document embedding, a process that moves beyond simple word statistics to capture the underlying semantic meaning of the text.  

This approach marks a significant advancement over the static word embedding methods discussed in Chapter 2, such as Word2Vec or GloVe~\cite{mikolov2013,pennington2014glove}. Those earlier models produce a single, fixed vector for each word, regardless of its context. This limitation makes them unable to distinguish between different meanings of the same word (polysemy) and often relies on crude heuristics like averaging word vectors, which can dilute or lose critical semantic information~\cite{JiangPardosMulti2VecEDM2020}.
