...
\begin{frame}
    ...
    \framesubtitle{The Transfer Maze}
...
\frametitle{The Experimental Environment}
    \begin{columns}[T]
        ...
        \begin{column}{0.25\textwidth}
            \centering
            \includegraphics[width=\textwidth]{GoogToSFSU.png}
        \end{column}
    \end{columns}
...
\begin{comment}
    \textbf{Notes for the presenter:}
    \begin{enumerate}
        \item Start by setting the stage: "Before we dive into the results, let's briefly cover the tools we used." This transitions smoothly from the previous slides.
        \item Emphasize the "hybrid approach." Explain that using Gemini was a pragmatic first step to set a baseline and understand the problem's scope, but the real work required a different, more powerful setup.
        \item When talking about the hardware, mention that this distinction allowed for efficient use of resources—GPUs for the deep learning fine-tuning and CPUs for the exhaustive searches required by classical ML models. This shows a thoughtful and efficient experimental design.
        \item For the software, briefly touch on why these libraries were chosen (e.g., "PyTorch and Hugging Face are the standard for modern deep learning, and scikit-learn is the go-to for classical machine learning"). This adds credibility and context.
        \item Use the proposed graphic to guide the audience through the two different computational pathways, making a potentially dry topic more engaging.
    \end{enumerate}
\end{comment}
