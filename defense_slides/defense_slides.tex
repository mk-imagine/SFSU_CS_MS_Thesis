...
\author{Mark S. Kim}
\institute[SFSU]{San Francisco State University \\ Department of Computer Science}
\date{August 6, 2025}
...
\begin{frame}
    \frametitle{Special Thanks To\ldots}
    \begin{columns}[T] % Splits the frame into columns
        
        \begin{column}{0.4\textwidth}
            \includegraphics[width=0.4\textwidth]{ppm.pdf}\\
            \vspace*{0.5cm}
            \includegraphics[width=\textwidth]{foundation_logo_notag_color.pdf}\\
            \vspace*{0.5cm}
            \includegraphics[width=0.45\textwidth]{ccc-logo-vfull-3c.png}
        \end{column}
        
        \begin{column}{0.4\textwidth} % Left column for text
            \begin{itemize}
                \item Dr. Craig Hayward
                \item The PPM Team
                \item The Foundation for California Community Colleges
                \item California Community Colleges Chancellor's Office
            \end{itemize}
        \end{column}
        
    \end{columns}
    % Add PPM Team, FCCC, CCC Chancellor's Office
...
\begin{frame}
    ...
    \begin{columns}[T] % Splits the frame into columns
        ...
        \begin{column}{0.6\textwidth} % Left column for text
            \begin{itemize}
                \item The process for determining course equivalency, or \textbf{articulation}, is a formidable, largely manual process that creates significant barriers for students~\cite{pardos10.1145/3330430.3333622}.
                ...
                \item This task of ``bleak combinatorics'' is inefficient, slow, and inherently intractable, struggling to keep pace with the needs of a vast and mobile student body~\cite{pardos10.1145/3330430.3333622}.
...
% The Goal & Our Contribution (Slide 1.5)
\begin{frame}
    \frametitle{The Goal \& Our Contribution}
    \vspace{-0.5em}
    \begin{columns}[T]
        \begin{column}{0.47\textwidth}
            \begin{block}{The Goal}
                ...
                \begin{itemize}
                    ...
                    \item Inherently Privacy-Preserving
                \end{itemize}
            \end{block}
        ...
        \begin{column}{0.47\textwidth}
            \begin{block}{Primary Contributions}
                \hfill
                \begin{enumerate}
                    \item \textbf{A Highly Accurate Framework:} Developed a complete pipeline achieving state-of-the-art accuracy on real-world data.\vspace{0.5em}
                    \item \textbf{An Innovative Feature Vector:} Designed a novel composite vector combining local and global semantics to improve classification.\vspace{0.5em}
                    \item \textbf{An Efficient \& Private Approach:} Created a decoupled solution that avoids the high costs and privacy concerns of prior methods.\vspace{1em}
                \end{enumerate}
            \end{block}
        \end{column}
    \end{columns}
...
\begin{frame}
    \frametitle{Related Work \& The Research Gap}
    ...
    \begin{columns}[T]
        ...
        \begin{column}{0.4\textwidth}
            \begin{alertblock}{The Opportunity}
                The limitations of direct LLM classification (cost, opacity) and enrollment-based methods (privacy, limited access) point toward a gap in the existing research for a new paradigm~\cite{pardos10.1145/3330430.3333622, slade10.1177/0002764213479366}.
                \vspace{1em}
                
                \textbf{An effective solution needs to harness the semantic power of large models without inheriting their operational burdens.}
            \end{alertblock}
        \end{column}

    \end{columns}

    \note[item]{This diagram clearly shows the trade-offs. In the bottom-left, we have simple methods with low semantic power. In the top-right, we have powerful methods that are either expensive or raise privacy concerns. The top-left quadrant is where we want to be: high semantic power, but with low cost and low/no privacy risk. This is the gap our research addresses.}
    \note[item]{mention Phase I (direct LLM classification)}
...
\begin{frame}
    ...
    \note[item]{Take course description text}
    \note[item]{Deep embedding models -> document embeddings}
    \note[item]{Reduce dimensions}
    \note[item]{Construct Composite Distance Vectors}
    \note[item]{Binary classification with traditional ML classifiers}
    
\end{frame}

% Core Principle: Decoupling Representation from Classification (Slide 3.2)
\begin{frame}
    \frametitle{Core Principle: Decoupling Data Representation from Classification}
    ...
    \begin{tikzpicture}[
        ...
        \node (stage1) [stage] {
            ...
            \\[2ex]
            A deep embedding model converts raw course texts into structured, reusable semantic vectors. A single course embedding is a relatively light task.
        };
    ...
    \note{This is one of the most important concepts in the framework. We do the heavy lifting of understanding language only once, offline. This lets us do the actual classification of pairs incredibly quickly and efficiently. This two-stage process is what gives us the best of both worlds: the semantic power of deep learning without the high operational costs for every comparison, which is what makes the system so scalable and practical.}
\end{frame}

\begin{frame}
\frametitle{Prompting Methods: From Verbose to Normalized}
\framesubtitle{The distinction between two approaches}
\begin{columns}
\begin{column}{0.5\textwidth}
    \textbf{Phase 1: Direct LLM Classification}
    \begin{itemize}
        \item \textbf{Method:} End-to-end classification using a large language model.
        \item \textbf{Input:} A single, lengthy prompt containing verbose instructions, few-shot examples, and the raw course descriptions.
        \item \textbf{Challenge:} Performance was highly sensitive to prompt phrasing and formatting, aligning with known limitations of LLMs. This method was not robust for a production system.
    \end{itemize}
\end{column}
\begin{column}{0.5\textwidth}
    \textbf{Phase 2: Decoupled Framework}
    \begin{itemize}
        \item \textbf{Method:} Generate embeddings from text, then classify.
        \item \textbf{Input:} Normalized course text (e.g., ``CSC-220 Data Structures\ldots''), with no external instructions.
        \item \textbf{Benefit:} The model learns semantic meaning directly from the text itself, making it robust to formatting changes and eliminating the need for complex prompt engineering. This is more suitable for a scalable system.
    \end{itemize}
\end{column}
\end{columns}
\end{frame}
...
\begin{frame}
    ...
    \frametitle{Step 2a: Our Novel Feature Vector (\(\Delta_c\))}
    ...
    \note[item]{These can be from pretrained "off the shelf" embedding models or a fine-tuned one.}
\end{frame}

% Step 3: Domain-Specific Fine-Tuning (Slide 3.5)
\begin{frame}
    \frametitle{Step 2b: Domain-Specific Fine-Tuning}
    ...
    \begin{columns}[T]
        \begin{column}{0.55\textwidth}
            \begin{alertblock}{Learning Objective: The Triplet Loss}
                We train the model using a \textbf{Triplet Loss} function, which teaches the model to understand nuanced similarity by operating on triplets of courses~\cite{Schroff_2015, hermans2017defensetripletlossperson}:
                \begin{itemize}
...
\begin{frame}
    \frametitle{Step 3: Downstream Classification}
    ...
    \begin{columns}[T]
        \begin{column}{0.57\textwidth}
            \hfill\vspace{3mm}
            \begin{block}{Classifier Model Evaluation}
                \begin{itemize}
                    \item Linear Models (e.g., Logistic Regression)
                    \item Kernel-Based Models (e.g., SVM)
                    \item Instance-Based Models (e.g., KNN)
                    \item Ensemble Tree Models (e.g., Random Forest)
                    \item Gradient Boosting Models (e.g., XGBoost)
                \end{itemize}
            \end{block}
        \end{column}
...
% Data & Evaluation (Slide 4.0.1)
\begin{frame}
\frametitle{The Experimental Environment}
    \begin{columns}[T]
        \begin{column}{0.57\textwidth}
            \begin{itemize}
                \item \textbf{Hybrid Computing Approach:} Utilized both a cloud-based LLM and a local High-Performance Computing (HPC) cluster.
                \item \textbf{Cloud for Baseline:} Google's Gemini v1.0 was used for the initial, exploratory direct LLM classification.
                \item \textbf{Local HPC for Core Work:} The SFSU POLARIS HPC cluster was used for all primary, computationally intensive experiments.
                \begin{itemize}
                    \item \textbf{Embedding \& Fine-Tuning:} GPU tasks on 4x NVIDIA A100s
                    \item \textbf{Classifiers:} CPU-bound tasks on AMD EPYC 9534 CPUs
                \end{itemize}
                \item \textbf{Software:} The pipeline was built with Python, PyTorch, Hugging Face, scikit-learn, and XGBoost.
            \end{itemize}
        \end{column}

        \begin{column}{0.38\textwidth}
            \centering
            \includegraphics[width=0.7\textwidth]{GoogToSFSU.png}
        \end{column}
    \end{columns}

    \note[item]{Before we dive into results, let's take a look at the environment we used\ldots}
    \note[item]{Mention that the initial feasibility study used a cloud-based service (Gemini) to quickly establish a performance baseline, but this approach was not viable for a scalable production system due to cost and latency.}
    \note[item]{Bulk of research including fine-tuning and model evaluations on the on-premise SFSU POLARIS cluster.}
    \note[item]{Embedding \& Fine-tuning were important for deep learning tasks, but scikit-learn is CPU bound}
    \note[item]{PyTorch -> Huggingface Transformers -> Sentence-Transformers dependency for embedding/fine-tuning; is the current standard for deep learning from Meta (the older being Tensorflow/Keras from Google)}
    \note[item]{scikit-learn the (current) gold standard in traditional machine learning (and XGBoost -- not part of scikit, but integrates well with it)}
\end{frame}
...
\begin{frame}
    ...
    \begin{tabularx}{0.95\textwidth}{>{\raggedright\arraybackslash}p{2.2cm} >{\raggedright\arraybackslash}X >{\raggedright\arraybackslash}X}
        ...
        \addlinespace
        \textbf{Final Size}     & 400 course pairs                                 & 9192 course pairs (test set)                  \\
        \addlinespace
        \textbf{Partitioning}   & Stratified random sample                                              & Stratified 50/50 train/test split                    \\
        \bottomrule
    \end{tabularx}

    \note[item]{Pipeline required several training and inference steps.}
    \note[item]{All training/fitting occurred on training set, including fine-tuning, dimension reducers, and classifiers}
    \note[item]{Fine-tuning (all done on train set): training on batch triplets, evaluation on constructed binary pairs (equiv/non-equiv)}
    \note[item]{Dim Reducers: fit on train, transform only on test}
    \note[item]{ZERO data leakage into final test set}
\end{frame}
...
\begin{frame}
    ...
    \note[item]{Adam vs AdamW: Adam has its weight decay added to loss, affecting adaptive learning rates; AdamW decouples this weight decay (applied to parameter update rather than to the loss)}

\end{frame}

% Finding 1: All Finalists Perform Exceptionally Well (Slide 4.3.1)
\begin{frame}
    \frametitle{Finding 1: All Finalist Classifiers Perform Exceptionally Well}
    ...
    \begin{columns}[T]
        ...
        \begin{column}{0.6\textwidth}
            ...
            \centering
            \includegraphics[width=0.95\textwidth]{classifier_f1score_boxplot_test.pdf} % Placeholder for Figure 4.4
...
% Finding 2: The Result (The Punchline) (Slide 4.2.2)
\begin{frame}
    \frametitle{Finding 2: Fine-Tuned Embedding Model is Statistically Superior}
...
% Finding 2: The Insight (The "So What?") (Slide 4.2.3)
\begin{frame}
...
% Finding : The Accuracy vs. Efficiency Trade-Off (Slide 4.3.2)
\begin{frame}
    ...
    \begin{columns}[T]
        ...
        \begin{column}{0.6\textwidth}
            ...
            \centering
            \includegraphics[width=0.95\textwidth]{Model_Score_vs_Time_Costs.pdf} % Placeholder for Figure 4.3
...
\begin{frame}
    ...
    \note[item]{This becomes a Data-Centric Problem}
    ...
    \begin{block}{Key Limitations}
        ...
        \begin{columns}[T]
            ...
            \begin{column}{0.3\textwidth}
                ...
                The framework simplifies articulation into a binary classification of course pairs and does not natively handle complex one-to-many or many-to-many agreements, a challenge that persists for many automated systems~\cite{pardos10.1145/3330430.3333622}.
            \end{column}
        \end{columns}
    \end{block}
...
\begin{frame}
    ...
    \begin{columns}[T]
        ...
        \begin{column}{0.45\textwidth}
            \begin{block}{Current Ongoing Work}
        ...
        \begin{column}{0.6\textwidth}
            \begin{block}{Acknowledgments}
                ...
                \begin{itemize}
                    ...
                    \item My colleagues: \textbf{Shreyas Raghuraman} for data preparation; \textbf{Jay Lodha} for insightful questions and comments; and \textbf{Natalie Yam}, \textbf{Parth Panchal}, and \textbf{Joanne Park} for early assistance with preliminary analysis.
                \end{itemize}
            \end{block}
        \end{column}
...
\begin{frame}
    ...
    & High computational cost, opaque "black box" reasoning, and sensitive to prompt phrasing~\cite{Errica2024WhatDI}.  Also empirically illustrated in our preliminary work (Phase I) \\
    
    \bottomrule
    \end{tabularx}
    
    \note[item]{This slide provides the full context for our work. The key takeaway is that each prior method had a significant drawback, whether it was a lack of semantic understanding, a reliance on private data, or operational complexity. This creates a clear research gap that our framework is designed to fill.}
    \note[item]{Did Direct LLM Classification in Phase I}
...
\begin{frame}
    ...
    \note[item]{This is strong evidence that the problem isn't the model's fault. In many cases, the models are correctly reporting that two course descriptions are not semantically similar. The issue is that the ground-truth label says they *should* be equivalent, pointing to an inconsistency in the source data itself. On the flip side, we may find two course descriptions that are indeed semantically similar, but are labeled as non-equivalent.}
